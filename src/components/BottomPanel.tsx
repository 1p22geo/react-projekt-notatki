...
export default function BottomPanel({
  ...
  const allExtractedLinks = useMemo(
    () => extractLinks(currentNote.content),
    [currentNote.content],
  );
  ...
  const backlinks = useMemo(() => {
    return allNotes.filter((note) => {
      ...
      return noteLinks.some(
        (link) =>
          !link.isExternal &&
          link.title.toLowerCase() === currentNote.title.toLowerCase(),
      );
    });
        ...
        <div className="h-52 overflow-y-auto p-6 bg-gray-50/50">
          {activeTab === "links" ? (
                ...
                links.map((link, i) => (
                  ...
                  >
                    <div className="flex items-center space-x-3 truncate">
                      <div
                        className={`p-1.5 rounded-lg ${link.isExternal ? "bg-emerald-50 text-emerald-600" : "bg-blue-50 text-blue-600"}`}
                      >
                        <LinkIcon size={14} />
                      </div>
                    ...
                    <ExternalLink
                      size={12}
                      className={`text-gray-300 group-hover:text-blue-400 opacity-0 group-hover:opacity-100 transition-all ${link.isExternal ? "opacity-100! text-emerald-300!" : ""}`}
                    />
                  </button>
                ))
              ) : (
              ...
              {backlinks.length > 0 ? (
                backlinks.map((note) => (
                  ...
                  >
                    ...
                    <ExternalLink
                      size={12}
                      className="text-gray-300 group-hover:text-indigo-400 opacity-0 group-hover:opacity-100 transition-all"
                    />
                  </button>
                ))
              ) : (
            ...
            <div className="max-w-md space-y-4">
              <h3 className="text-sm font-bold text-gray-800 uppercase tracking-widest px-1">
                Document Type
              </h3>
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-4 rounded-2xl border transition-all ${
                    currentNote.type === "note"
                      ? "bg-blue-50 border-blue-200 ring-2 ring-blue-100 shadow-sm"
                ...
                >
                  ...
                  />
                  <span
                    className={`text-xs font-bold mt-2 ${currentNote.type === "note" ? "text-blue-700" : "text-gray-500"}`}
                  >
                    Text note
                  </span>
                </button>
                ...
                >
                  ...
                  />
                  <span
                    className={`text-xs font-bold mt-2 ${currentNote.type === "place" ? "text-emerald-700" : "text-gray-500"}`}
                  >
                    Place
                  </span>
                </button>
